...
function _photoQueueAdd(entry){
  // De-dupes by capType+row-key before adding - fixed during Task 5's code review: a Refill
  ...
  // Row-key uses rowRids when present, falling back to [rowRid] - fixed during Task 6's code
  // review: comparing entry.rowRid directly broke for Received (Task 6), which only ever
  // sets rowRids (plural), never rowRid - every Received queue entry had rowRid===undefined,
  // so ANY two Received entries queued before the first one drained (e.g. two deliveries
  // committed back-to-back) matched each other on that shared undefined and silently evicted
  // one another, permanently losing whichever delivery's photos got evicted (no error, no
  // retry - the entry was just gone). JSON.stringify-ing the normalized rowRids array instead
  // means two DIFFERENT deliveries (different _rid()-generated ids, no overlap) never match,
  // while a genuine same-row recommit (identical id or identical array) still correctly
  // replaces rather than duplicates.
  var _rowKey=function(e){return JSON.stringify(e.rowRids||[e.rowRid]);};
  var newKey=_rowKey(entry);
  var q=_photoQueueLoad().filter(function(e){return !(e.capType===entry.capType && _rowKey(e)===newKey);});
